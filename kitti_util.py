...
def draw_projected_box3d(image, qs, color=(255,255,255), thickness=2):
    ...
    cv2.line(image, (qs[1,0],qs[1,1]), (qs[4,0],qs[4,1]), (255,0,0), thickness, cv2.LINE_AA)
    cv2.line(image, (qs[0,0],qs[0,1]), (qs[5,0],qs[5,1]), (255,0,0), thickness, cv2.LINE_AA)
...
def compute_3D_line(x1,x2, y1,y2, z1,z2):
    
    line_x1_x2 = compute_line(x1, x2, d=300)
    line_y1_y2 = compute_line(y1, y2, d=300)
    line_z1_z2 = compute_line(z1, z2, d=300)    
    
    save_points = np.zeros((7,300), dtype = np.float)
    save_points[0,:] = line_x1_x2
    save_points[1,:] = line_y1_y2
    save_points[2,:] = line_z1_z2
    save_points[4,:] = 255
    return save_points
    
def compute_3D_line_(x1,x2, y1,y2, z1,z2):
...
def compute_line(x1, x2, d=300):
    ...
    if x1 == x2:
        return np.ones((300,), dtype = np.float) * x1
    elif x1 < x2:
        return np.arange(x1, x2, (x2 - x1)/d)
    elif x1 > x2: 
        tmp = np.arange(x2, x1, (x1 - x2)/d)
        return tmp[::-1]
